...
def test_loads_vivo_update_target_from_android_package_name(tmp_path: Path) -> None:
    application = load_config(_write(tmp_path, VIVO)).apps["wallet"]

    target = resolve_store_target(application, StoreName.VIVO)
...
@pytest.mark.parametrize(
    "invalid",
    [
        VIVO.replace("credential_profile: vivo-wallet", "credential_profile: ' '"),
        VIVO.replace("version_code: 124", "version_code: 0"),
        VIVO.replace("version_code: 124", "version_code: -1"),
        VIVO.replace("version_code: 124", "version_code: '124'"),
        VIVO.replace("version_code: 124", "version_code: true"),
        VIVO.replace("language: zh-CN", "language: zh_CN"),
        VIVO + "        unexpected: true\n",
    ],
)
def test_vivo_configuration_is_strict(tmp_path: Path, invalid: str) -> None:
    with pytest.raises(ConfigError, match="vivo"):
